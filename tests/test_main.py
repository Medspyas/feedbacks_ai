...
async def test_full_feedback_success(client):
    ...
    with patch("app.services.feedback_services.FeedbackServices.create_feedback", new_callable=AsyncMock) as mock_create, \
        ...
        updated_data = {**payload, "id": "id_123", "content": "Le contenu a été modifié pour le test."}
...
async def test_get_one_feedback_not_found(client):
    ...
    with patch("app.services.feedback_services.FeedbackServices.get_one_feedback", new_callable=AsyncMock) as mock_get_one:
...
async def test_update_feedback_not_found(client):
    ...
    with patch("app.services.feedback_services.FeedbackServices.update_feedback", new_callable=AsyncMock) as mock_update:
...
async def test_delete_feedback_not_found(client):
    ...
    with patch("app.services.feedback_services.FeedbackServices.delete_feedback", new_callable=AsyncMock) as mock_delete:
...
async def test_get_feedbacks_count(client):
    with patch("app.services.feedback_services.FeedbackServices.count_feedbacks", new_callable=AsyncMock) as mock_count:
...
async def test_get_feedbacks_pagination(client):
    with patch("app.services.feedback_services.FeedbackServices.get_all_feedbacks", new_callable=AsyncMock) as mock_get_all:
        mock_get_all.return_value = [{"username": f"user{i}", "company_name": "test", "category": "test", "content": "test_content", "rating": 3} for i in range(10)]
...
async def test_search_feedbacks(client):
    with patch("app.repositories.feedback_repo.FeedbackRepository.search", new_callable=AsyncMock) as mock_search:
        mock_search.return_value = [{"username": "medsfire", "company_name": "test", "category": "test", "content": "test_content", "rating": 3}]
