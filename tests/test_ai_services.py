...
async def test_ai_service_success():
    ...
    with patch.object(ai.client.chat.completions, "create", new_callable=AsyncMock) as mock_create:
...
async def test_ai_service_fallback_model_success():
    ...
    mock_choice = MagicMock()
    mock_choice.message.content = json.dumps({"sentiment": "Réponse du modèle de secours"})
